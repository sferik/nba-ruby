...
module NBA
  class LeagueDashPlayerStatsRecordAttributeMappingTest < Minitest::Test
    cover LeagueDashPlayerStats

    def test_maps_games_played
    ...
    def test_maps_losses
      stub_player_stats_request

      stat = LeagueDashPlayerStats.all(season: 2024).first

      assert_equal 26, stat.l
    end

    def test_maps_win_percentage
      stub_player_stats_request

      stat = LeagueDashPlayerStats.all(season: 2024).first

      assert_in_delta 0.639, stat.w_pct
    end
