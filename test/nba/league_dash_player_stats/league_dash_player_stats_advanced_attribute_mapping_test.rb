...
module NBA
  class LeagueDashPlayerStatsAdvancedAttributeMappingTest < Minitest::Test
    cover LeagueDashPlayerStats

    def test_maps_all_advanced_attributes
      stub_player_stats_request

      stat = LeagueDashPlayerStats.all(season: 2024).first

      assert_in_delta 5.2, stat.plus_minus
      assert_in_delta 45.6, stat.nba_fantasy_pts
      assert_equal 12, stat.dd2
      assert_equal 0, stat.td3
    end
