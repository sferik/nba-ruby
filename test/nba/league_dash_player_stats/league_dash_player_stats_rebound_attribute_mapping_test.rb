...
module NBA
  class LeagueDashPlayerStatsReboundAttributeMappingTest < Minitest::Test
    cover LeagueDashPlayerStats

    def test_maps_offensive_rebounds
    ...
    def test_maps_defensive_rebounds
      stub_player_stats_request

      stat = LeagueDashPlayerStats.all(season: 2024).first

      assert_in_delta 4.3, stat.dreb
    end

    def test_maps_total_rebounds
      stub_player_stats_request

      stat = LeagueDashPlayerStats.all(season: 2024).first

      assert_in_delta 4.8, stat.reb
    end
