...
module NBA
  class LeagueDashOppPtShotAllTest < Minitest::Test
    cover LeagueDashOppPtShot

    def test_all_returns_collection
      stub_request(:get, /leaguedashoppptshot/).to_return(body: pt_shot_response.to_json)

      assert_instance_of Collection, LeagueDashOppPtShot.all(season: 2024)
    end

    def test_all_uses_correct_result_set
      stub_request(:get, /leaguedashoppptshot/).to_return(body: pt_shot_response.to_json)

      result = LeagueDashOppPtShot.all(season: 2024)
    ...
    def test_all_with_season_type_parameter
      ...
      LeagueDashOppPtShot.all(season: 2024, season_type: LeagueDashOppPtShot::PLAYOFFS)
    ...
    def test_all_with_per_mode_parameter
      ...
      LeagueDashOppPtShot.all(season: 2024, per_mode: LeagueDashOppPtShot::TOTALS)
    ...
    def test_all_with_custom_client
      ...
      LeagueDashOppPtShot.all(season: 2024, client: mock_client)
