...
module NBA
  class TeamHistoricalLeadersRebValuesTest < Minitest::Test
    ...
    def test_extracts_reb
      ...
      assert_equal 4_874, leader.reb
    end

    def test_extracts_reb_person_id
      stub_request(:get, /teamhistoricalleaders/).to_return(body: leaders_response.to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_equal 201_142, leader.reb_person_id
    end

    def test_extracts_reb_player
      stub_request(:get, /teamhistoricalleaders/).to_return(body: leaders_response.to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_equal "Kevin Durant", leader.reb_player
    end
