...
module NBA
  class PlayerVsPlayerTest < Minitest::Test
    ...
    def test_overall_returns_collection
      ...
      assert_instance_of Collection, PlayerVsPlayer.overall(player: 201_939, vs_player: 201_566)
    end

    def test_overall_uses_correct_result_set
      stub_request(:get, /playervsplayer/).to_return(body: response_with_both_result_sets.to_json)

      assert_equal "Overall", PlayerVsPlayer.overall(player: 201_939, vs_player: 201_566).first.court_status
    end

    def test_on_off_court_returns_collection
      stub_request(:get, /playervsplayer/).to_return(body: on_off_court_response.to_json)

      assert_instance_of Collection, PlayerVsPlayer.on_off_court(player: 201_939, vs_player: 201_566)
    end

    def test_on_off_court_uses_correct_result_set
      stub_request(:get, /playervsplayer/).to_return(body: response_with_both_result_sets.to_json)

      assert_equal "On", PlayerVsPlayer.on_off_court(player: 201_939, vs_player: 201_566).first.court_status
    end
    ...
    def test_returns_empty_for_nil_response
      ...
      assert_empty PlayerVsPlayer.overall(player: 201_939, vs_player: 201_566, client: mock_client)
      mock_client.verify
    end
    ...
    def row_data(status)
      [201_939, 201_566, status, 24, 32.5, 26.4, 5.7, 6.1, 1.2, 0.3, 3.1, 0.467, 8.5]
    end
