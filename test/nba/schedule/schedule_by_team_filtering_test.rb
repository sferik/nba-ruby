require_relative "../../test_helper"

module NBA
  class ScheduleByTeamFilteringTest < Minitest::Test
    cover Schedule

    def test_by_team_finds_games_when_team_is_home
      stub_schedule_request

      games = Schedule.by_team(team: Team::GSW)

      assert_equal 1, games.size
      assert_equal Team::GSW, games.first.home_team_id
    end

    def test_by_team_finds_games_when_team_is_away
      stub_schedule_request

      games = Schedule.by_team(team: Team::LAL)

      assert_equal 1, games.size
      assert_equal Team::LAL, games.first.away_team_id
    end

    def test_by_team_returns_empty_when_team_is_neither_home_nor_away
      stub_schedule_request

      games = Schedule.by_team(team: Team::BOS)

      assert_equal 0, games.size
    end

    private

    def stub_schedule_request
      stub_request(:get, /scheduleleaguev2/).to_return(body: schedule_response.to_json)
    end

    def schedule_response
      {leagueSchedule: {gameDates: [{gameDate: "2024-10-22", games: [game_data]}]}}
    end

    def game_data
      {gameDateTimeUTC: "2024-10-22T19:00:00Z", gameId: "0022400001",
       homeTeam: {teamId: Team::GSW, teamName: "Warriors"},
       awayTeam: {teamId: Team::LAL, teamName: "Lakers"}}
    end
  end
end
