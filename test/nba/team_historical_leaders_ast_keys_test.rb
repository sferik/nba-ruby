...
module NBA
  class TeamHistoricalLeadersAstKeysTest < Minitest::Test
    cover TeamHistoricalLeaders

    def test_handles_missing_ast_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("AST").to_json)
    ...
    def test_handles_missing_ast_person_id_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("AST_PERSON_ID").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.ast_person_id
    end

    def test_handles_missing_ast_player_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("AST_PLAYER").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.ast_player
    end
