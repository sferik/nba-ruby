...
module NBA
  class TeamHistoricalLeadersRebKeysTest < Minitest::Test
    cover TeamHistoricalLeaders

    def test_handles_missing_reb_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("REB").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.reb
    end

    def test_handles_missing_reb_person_id_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("REB_PERSON_ID").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.reb_person_id
    end

    def test_handles_missing_reb_player_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("REB_PLAYER").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.reb_player
    end
