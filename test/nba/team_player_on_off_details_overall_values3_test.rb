...
module NBA
  class TeamPlayerOnOffDetailsOverallValues3Test < Minitest::Test
    ...
    def test_extracts_oreb_rank
      assert_equal 20, @result.oreb_rank
    end

    def test_extracts_dreb_rank
      assert_equal 11, @result.dreb_rank
    end

    def test_extracts_reb_rank
      assert_equal 13, @result.reb_rank
    end

    def test_extracts_ast_rank
      assert_equal 4, @result.ast_rank
    end

    def test_extracts_tov_rank
      assert_equal 18, @result.tov_rank
    end

    def test_extracts_stl_rank
      assert_equal 7, @result.stl_rank
    end

    def test_extracts_blk_rank
      assert_equal 19, @result.blk_rank
    end
    ...
    def test_extracts_pf_rank
      assert_equal 25, @result.pf_rank
    end

    def test_extracts_pfd_rank
      assert_equal 17, @result.pfd_rank
    end

    def test_extracts_pts_rank
      assert_equal 2, @result.pts_rank
    end

    def test_extracts_plus_minus_rank
      assert_equal 6, @result.plus_minus_rank
    end

    private

    def response
      {resultSets: [{name: "OverallTeamPlayerOnOffDetails", headers: headers, rowSet: [row]}]}
    end
