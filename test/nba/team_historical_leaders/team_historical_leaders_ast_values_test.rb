...
module NBA
  class TeamHistoricalLeadersAstValuesTest < Minitest::Test
    ...
    def test_extracts_ast
      ...
      assert_equal 6_475, leader.ast
    end

    def test_extracts_ast_person_id
      stub_request(:get, /teamhistoricalleaders/).to_return(body: leaders_response.to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_equal 201_939, leader.ast_person_id
    end

    def test_extracts_ast_player
      stub_request(:get, /teamhistoricalleaders/).to_return(body: leaders_response.to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_equal "Stephen Curry", leader.ast_player
    end
