...
module NBA
  class TeamHistoricalLeadersIdentityKeysTest < Minitest::Test
    cover TeamHistoricalLeaders

    def test_handles_missing_team_id_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("TEAM_ID").to_json)
    ...
    def test_handles_missing_season_year_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("SEASON_YEAR").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.season_year
    end
