...
module NBA
  class TeamHistoricalLeadersBlkValuesTest < Minitest::Test
    ...
    def test_extracts_blk
      ...
      assert_equal 986, leader.blk
    end

    def test_extracts_blk_person_id
      stub_request(:get, /teamhistoricalleaders/).to_return(body: leaders_response.to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_equal 101_106, leader.blk_person_id
    end

    def test_extracts_blk_player
      stub_request(:get, /teamhistoricalleaders/).to_return(body: leaders_response.to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_equal "Andrew Bogut", leader.blk_player
    end
