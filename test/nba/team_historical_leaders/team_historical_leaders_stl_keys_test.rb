...
module NBA
  class TeamHistoricalLeadersStlKeysTest < Minitest::Test
    cover TeamHistoricalLeaders

    def test_handles_missing_stl_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("STL").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.stl
    end

    def test_handles_missing_stl_person_id_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("STL_PERSON_ID").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.stl_person_id
    end

    def test_handles_missing_stl_player_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("STL_PLAYER").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.stl_player
    end
