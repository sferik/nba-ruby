...
module NBA
  class TeamHistoricalLeadersPtsKeysTest < Minitest::Test
    cover TeamHistoricalLeaders

    def test_handles_missing_pts_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("PTS").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.pts
    end

    def test_handles_missing_pts_person_id_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("PTS_PERSON_ID").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.pts_person_id
    end

    def test_handles_missing_pts_player_key
      stub_request(:get, /teamhistoricalleaders/).to_return(body: response_missing_key("PTS_PLAYER").to_json)

      leader = TeamHistoricalLeaders.find(team: 1_610_612_744)

      assert_nil leader.pts_player
    end
