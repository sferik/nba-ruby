...
module NBA
  ...
  module LeagueDashPtDefend
    ...
    def self.find_result_set(data)
      ...
      result_sets.find { |rs| rs.fetch("name", nil).eql?(LEAGUE_DASH_PT_DEFEND) }
    end
    ...
    def self.identity_info(data)
      {player_id: data.fetch("CLOSE_DEF_PERSON_ID", nil), player_name: data.fetch("PLAYER_NAME", nil),
       team_id: data.fetch("PLAYER_LAST_TEAM_ID", nil),
       team_abbreviation: data.fetch("PLAYER_LAST_TEAM_ABBREVIATION", nil),
       player_position: data.fetch("PLAYER_POSITION", nil), age: data.fetch("AGE", nil),
