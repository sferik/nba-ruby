  ...
  module PlayerDashPtShots
    ...
    def self.identity_info(data)
      {player_id: data.fetch("PLAYER_ID", nil), player_name_last_first: data.fetch("PLAYER_NAME_LAST_FIRST", nil),
       sort_order: data.fetch("SORT_ORDER", nil), gp: data.fetch("GP", nil), g: data.fetch("G", nil)}
    end
    ...
    def self.shot_info(data)
      {shot_type: data.fetch("SHOT_TYPE", nil), fga_frequency: data.fetch("FGA_FREQUENCY", nil),
       fgm: data.fetch("FGM", nil), fga: data.fetch("FGA", nil), fg_pct: data.fetch("FG_PCT", nil),
       efg_pct: data.fetch("EFG_PCT", nil)}
    end
